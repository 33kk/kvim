return function(layers)
	...
	utils.set_maps({
		...
		{ "", { "<leader><A-h>", "<leader><A-Left>" }, "<cmd>wincmd H<CR>", { desc = "Move window to the left side" } },
		...
		{ "", { "<leader><A-l>", "<leader><A-Right>" }, "<cmd>wincmd L<CR>", { desc = "Move window to the right side" } },

		-- Splits
		{ "", "<leader>h", ":split<Space>", { desc = "Open a horizontal split", silent = false } },
		{ "", "<leader>v", ":vsplit<Space>", { desc = "Open a vertical split", silent = false } },
		...
		{ "", { "j", "<Down>" }, 'v:count || mode(1)[0:1] == "no" ? "j" : "gj"', { desc = "Move down", expr = true } },
		{ "", { "k", "<Up>" }, 'v:count || mode(1)[0:1] == "no" ? "k" : "gk"', { desc = "Move up", expr = true } },

		-- Insert mode movement
		{ "i", "<C-a>", "<Home>", { desc = "I: same as 0" } },
		{ "i", "<C-h>", "<Left>", { desc = "I: same as h" } },
		{ "i", "<C-j>", "<Down>", { desc = "I: same as j" } },
		{ "i", "<C-k>", "<Up>", { desc = "I: same as k" } },
		{ "i", "<C-l>", "<Right>", { desc = "I: same as l" } },
		{ "i", "<C-d>", "<End>", { desc = "I: same as $" } },
		...
		{ "n", "<space>e", "<cmd>lua vim.diagnostic.open_float()<CR>", { desc = "" } },
		{ "n", "[d", "<cmd>lua vim.diagnostic.goto_prev()<CR>", { desc = "" } },
		{ "n", "]d", "<cmd>lua vim.diagnostic.goto_next()<CR>", { desc = "" }},
		{ "n", "<space>q", "<cmd>lua vim.diagnostic.setloclist()<CR>", { desc = "" }},
